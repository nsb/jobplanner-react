...
class InvoiceBatchClient extends Component<Props> {
  render() {
    ...
    return (
      <ListItem direction="column" align="start" separator="none">
        <Box colorIndex="light-2" pad="medium" full={"horizontal"}>
          <Box direction="row">
            <CheckBox
          ...
          <List onMore={undefined}>
            {(clientSelection && clientSelection.selected ? jobs : []).map(
              (job: Job, index) => {
                return (
                  <InvoiceBatchJobContainer
                    ...
                    selected={
                      new Map([
                        [
                          job.id,
                          clientSelection && clientSelection.jobs.get(job.id)
                        ]
                      ])
                    }
        ...
        new Map([
          [
            client.id,
            {
              ...
              ...{
                selected:
                  !(clientSelection && clientSelection.selected) || false
              }
            }
          ]
        ])
    ...
    const newSelected = new Map([
      [
        client.id,
        {
          ...
          jobs: new Map([
            ...((clientSelection && clientSelection.jobs) || new Map()),
            ...selection
          ])
        }
      ]
