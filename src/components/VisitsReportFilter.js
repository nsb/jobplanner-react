// @flow

import React, { Component } from 'react';
import Layer from 'grommet/components/Layer';
import Sidebar from 'grommet/components/Sidebar';
import Header from 'grommet/components/Header';
import Button from 'grommet/components/Button';
import Heading from 'grommet/components/Heading';
import Select from 'grommet/components/Select';
import CloseIcon from 'grommet/components/icons/base/Close';
import Form from "grommet/components/Form";
import FormFields from "grommet/components/FormFields";
import FormField from "grommet/components/FormField";
import DateTime from "grommet/components/DateTime";
import CheckBox from "grommet/components/CheckBox";
import Footer from "grommet/components/Footer";
import type { Employee } from "../actions/employees";

type Props = {
  onClose: Function,
  employees: Array<Employee>
};

type State = {
  begins: Date,
  ends: Date,
  complete: boolean,
  incomplete: boolean,
  assigned: Array<Number>
}

class VisitsReportFilter extends Component<Props, State> {
  static defaultProps = {
    employees: []
  };

  constructor(props: Props) {
    super(props);
    this.state = {
      begins: new Date(),
      ends: new Date(),
      complete: true,
      incomplete: false,
      assigned: []
    }
  }

  render () {
    const dateFormat = "M/D/YYYY";
    const { employees } = this.props;

    return (

      <Layer align='right' flush={true} closer={false}
        a11yTitle='Visits Report Filter'>
        <Sidebar size='medium'>
          <div>
            <Header size='small' justify='between' align='center'
              pad={{ horizontal: 'medium', vertical: 'none' }}>
              <Heading tag='h3' margin='none'>Filter</Heading>
              <Button icon={<CloseIcon />} plain={true}
                onClick={this.props.onClose} />
            </Header>
            <Form pad='medium'>
              <FormFields>
                <FormField label='From date'>
                  <DateTime id='begins'
                    name='begins'
                    format={dateFormat}
                    onChange={undefined}
                    value={this.state.begins} />
                </FormField>
                <FormField label='To date'>
                  <DateTime id='ends'
                    name='ends'
                    format={dateFormat}
                    onChange={undefined}
                    value={this.state.ends} />
                </FormField>
                <FormField label='Status'>
                  <CheckBox label='Complete' value={this.state.complete} />
                  <CheckBox label='Incomplete' />
                </FormField>
                <FormField label='Assigned to'>
                  <Select
                    placeHolder='All'
                    options={employees.map(employee => {
                      return { value: employee.id, label: employee.username };
                    })}
                    inline={false}
                    multiple={true}
                    value={this.state.assigned}
                    onChange={this.onEmployeesChange} />
                </FormField>
              </FormFields>
              <Footer pad={{"vertical": "medium"}}>
                <Button label='Apply'
                  type='submit'
                  primary={true}
                  onClick={undefined} />
              </Footer>
            </Form>
          </div>
        </Sidebar>
      </Layer>
    );
  }

  onEmployeesChange = ({target, option, value}) => {
    this.setState({assigned: value})
  }
}

export default VisitsReportFilter;
